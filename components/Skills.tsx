...
const Skills: React.FC = () => {
  ...
  const displayedSkills = SKILLS.filter(skill => content?.skills?.includes(skill.name));

  if (loading) {
    return (
        <section id="skills" className="py-24 bg-slate-100 dark:bg-secondary/30 rounded-lg animate-pulse">
            <h2 className="h-10 bg-slate-200 dark:bg-slate-700 rounded w-80 mx-auto mb-12"></h2>
  ...
  if (displayedSkills.length === 0) {
      return null; // No render the section if no skills are selected
  }
